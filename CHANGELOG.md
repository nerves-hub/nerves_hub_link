...
* Added
  * `NervesHubLink.Extensions.Alarms`, a new extension that sends the device's alarms to NervesHub as they are raised and cleared, rather than in the next health report, which can be an hour away. When NervesHub attaches it, it sends every alarm currently set, which corrects anything that changed while the device was offline. Each alarm carries the time it was raised or cleared, and alarms are watched from application start, so one raised during boot is dated to when it was raised rather than to when the device connected. Times are sent only once `NervesTime.synchronized?/0` says the clock is right. Alarms are named and filtered as Health names and filters them, and Health leaves them out of its reports while this extension is attached. Off by default: name it in `extension_modules` to turn it on. Requires a NervesHub with the Alarms extension. See the [extensions guide](guides/extensions.md#alarms).
