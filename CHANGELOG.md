...
* Added
  * Device-managed updates: a device can ask NervesHub for firmware when the moment suits it rather than taking whatever its deployment group sends. `NervesHubLink.check_for_update/1` reports what is waiting, `NervesHubLink.start_update/1` fetches and applies it, and `NervesHubLink.update_mode/1,2` moves between `:automatic` and `:device_managed`. Requires a NervesHub that supports them; against an older server nothing changes. See the [device-managed updates guide](guides/device_managed_updates.md). (#473)
  * `NervesHubLink.UpdateManager.PartsUpdater`, an update strategy that downloads firmware one part at a time and checks each against the checksums NervesHub sends, so a failed part is refetched on its own and a restart resumes from the last verified part. Set `max_concurrent_parts` to fetch several at once. See [choosing how firmware is downloaded and applied](guides/configuration.md#choosing-how-firmware-is-downloaded-and-applied). (#462)
  * `NervesHubLink.Extensions.ErrorReports`, which sends crashes, exits and errors your code handled (`NervesHubLink.report_error/3`) to NervesHub, where they are grouped into issues. Off by default. See [Error Reports](guides/extensions.md#error-reports). (#474)
  * `NervesHubLink.Extensions.Logging`, which sends log lines to NervesHub to be stored against the device and searched. In early release and off by default. See [Logging](guides/extensions.md#logging). (#303)
  * `NervesHubLink.Extensions.Logging.Batched`, version 0.1.0 of the logging extension, which sends a minute of lines per message instead of a message per line so a device stops losing most of its logs to the rate limit, and adds OTP reports. See [Sending a minute at a time](guides/extensions.md#sending-a-minute-at-a-time). (#470)
  * `NervesHubLink.Extensions.NetworkIdentity`, which reports the identities a device holds on networks NervesHub doesn't run — iroh, NetBird, Tailscale, WireGuard — so an operator can reach it by another route. Needs at least one configured provider. See [Network Identity](guides/extensions.md#network-identity). (#460)
  * Messages can be encoded as Msgpack instead of JSON, for fewer bytes on the wire and less memory encoding them. Needs the optional `msgpax` dependency and a server that supports it. See [Message serialization](guides/configuration.md#message-serialization). (#443)
  * `NervesHubLink.Supervisor`, a dedicated supervisor for the connection processes that you can add to your own tree with `config :nerves_hub_link, start: :manual`. The default start is unchanged. See [Supervision](guides/configuration.md#supervision). (#467)
  * `NervesHubLink.Message.UpdateInfo` now carries the `size`, `checksum` and `partials_checksums` NervesHub sends with the firmware URL, which were previously dropped while parsing the payload. (#462)
  * `NervesHubLink.Downloader` accepts `:range_end`, which pairs with `:resume_from_bytes` to fetch one slice of a file, and `:label`, which tags every line it logs so concurrent downloads can be told apart. (#462)
  * `NervesHubLink.UpdateManager.Updater` has an optional `c:NervesHubLink.UpdateManager.Updater.handle_message/2` callback for messages an updater sends itself and exits that aren't from its download. (#462)
  * Shared Secret authentication now recovers from a wrong device clock, which new devices often have on first boot, by signing with the `Date` header from the failed response until the device clock catches up.
  * `Nerves.Runtime.firmware_slots/0` is used for the auto-revert check when the installed `nerves_runtime` provides it, falling back to `nerves_fw_active` otherwise. (#442)

* Changed
  * Firmware and archives that cannot have their signature checked are now refused rather than applied. `fwup` verifies nothing when given no public key, so a device holding no `fwup_public_keys` was installing whatever it was sent, and an empty key list arriving over the socket no longer clears the keys a device already holds. (#464)
  * Extensions are now negotiated with the server rather than offered blind: the device offers back only what NervesHub advertises and it also implements. A server that advertises nothing is offered everything, as before. (#468)
  * Extensions are registered by name and version rather than by name alone, so more than one version can be implemented at once. (#468)
  * Joining the console channel now sends only `console_version` and `device_api_version`. The device channel has already sent the rest. (#454)
  * The minimum supported Elixir version is now 1.15, and CI covers up to Elixir 1.20.3 on OTP 29. (#432, #456)
  * The update path, socket and extensions are now covered by tests. (#453)
  * Bumped `castore` to 1.0.21, `mint` to 1.9.3, `nerves_runtime` to 0.13.13, `nerves_time` to 0.4.12, `plug_crypto` to 2.2.0, `vintage_net` to 0.13.12, `alarmist` to 0.4.2, and dev/test deps `bandit` (1.12.5), `credo` (1.7.19) and `ex_doc` (0.40.3).

* Fixed
  * `NervesHubLink.Downloader` no longer writes credentials to the log. Firmware URLs are signed, and the query string was written out when resuming or following a redirect, and reported along with SSL options and proxy headers in OTP's crash report. (#462)
  * A download resumed from a partial file asked for the wrong byte range after a disconnect and appended what came back in the wrong place, silently corrupting the firmware. This affected `NervesHubLink.UpdateManager.CachingUpdater` whenever a resumed download was interrupted a second time. (#462)
  * `NervesHubLink.UpdateManager.Updater` ignored the `{:stop, reason, state}` return that `c:NervesHubLink.UpdateManager.Updater.handle_downloader_message/2` documents, crashing with a `CaseClauseError` instead of stopping. (#462)
  * `NervesHubLink.UpdateManager.CachingUpdater` never cleared its cache on a rejected range request, because the `Downloader` wraps status errors in a `Mint.HTTPError` that the clause matching a bare `{:http_error, 404}` tuple never saw, so a stale partial was retried forever. A 416 now clears the cache too. (#461)
