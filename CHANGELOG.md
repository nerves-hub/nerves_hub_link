...
* Added
  ...
  * `NervesHubLink.Extensions.Logging.Batched`, version 0.1.0 of the logging extension. It collects log lines from application start and sends a minute of them per message rather than a message per line, which is what stops a device losing most of what it writes to NervesHub's rate limit. It lives alongside the 0.0.1 extension: name both in `extension_modules` and the device offers whichever version your NervesHub has. See the [extensions guide](guides/extensions.md#sending-a-minute-at-a-time).
